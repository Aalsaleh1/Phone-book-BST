...
public Scanner input=new Scanner(System.in); //check
    ...
    public void insert(Contact c) {
        root = insertRecursive(root, c.getContactName() ,c);
        System.out.print(c.getContactName());
    }

    // A recursive function to insert a new key and data into the BST
    private BSTNode insertRecursive(BSTNode root, String key, Contact data) {
    ...
    public Contact searchByname(String key)  { 
        if (recursive_search_name( key) != null)
            return recursive_search_name( key); 
        else
            return null;
    }
   
    private Contact recursive_search_name(String key)  { 
    	BSTNode p=root , q=root;;
    ...
    public Contact searchByphone(String num)  { 
    	 int x = 0;
    	    found_phone = null;  // Initialize found_phone to null before each search
    	    inorder(num, x);
    	    return found_phone;  // Return the found_phone after the search
    }
    ...
    public void searchBybirthday(String birthday) {
    	...
    	if(found) {
    		...
    		System.out.println("there is no contact with this birthday : "+ birthday);//1
    }
